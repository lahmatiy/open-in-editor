...
module.exports = {
  ...
  detectFor: function (platforms, editor) {
    return function() {
      // we use only system parts so we haven't to check anything except os
      if (~platforms.indexOf(process.platform)) {
        return Promise.resolve();
      }

      Promise.reject('"Open in ' + editor + '" does not implemented for your platform (' + process.platform + ')');
    };
  }
};
